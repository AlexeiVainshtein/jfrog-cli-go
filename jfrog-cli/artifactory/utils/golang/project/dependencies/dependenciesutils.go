...
func createDependencyInTemp(zipPath string) (path string, err error) {
...
func downloadModFileFromArtifactoryToLocalCache(cachePath, targetRepo, name, version string, details *config.ArtifactoryDetails, client *httpclient.HttpClient) string {
	...
	if err != nil {
		log.Error("Received an error:", err, name, version)
		return ""
	}
	...
	if dirExists {
		...
		auth, err := details.CreateArtAuthConfig()
		if err != nil {
			log.Error("Received an error creating config:", err)
			return ""
		}
...
func GetRegex() (regExp *RegExp, err error) {
	...
	editedByJFrogCli, err := utils.GetRegExp(`^(// Generated by)`)
	if err != nil {
		return
	}
